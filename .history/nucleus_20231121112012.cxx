...
NucleusPtr Nucleus::create(const std::string& species, double nucleon_dmin) {
  ...
  if (species == "p")
    ...
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
       63, 4.20, 0.596, 0.162, -0.006, 0.0, nucleon_dmin
    }};
  else if (species == "Xe")
    return NucleusPtr{new WoodsSaxonNucleus{
    ...
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      129, 5.36, 0.590, 0.162, -0.003, 0.0, nucleon_dmin
    }};
  else if (species == "Au")
    return NucleusPtr{new WoodsSaxonNucleus{
    ...
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      197, 6.38, 0.535, -0.131, -0.031, 0.0, nucleon_dmin
    }};
  else if (species == "Pb")
    return NucleusPtr{new WoodsSaxonNucleus{
      208, 6.65, 0.54, nucleon_dmin// this is the original one, before changed for luuk 208, 6.62, 0.546, nucleon_dmin
    }};
  else if (species == "U")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      238, 6.81, 0.600, 0.280, 0.093, 0.0, nucleon_dmin
    }};
  else if (species == "U2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      238, 6.86, 0.420, 0.265, 0.000, 0.0, nucleon_dmin
    }};
  else if (species == "U3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      238, 6.67, 0.440, 0.280, 0.093, 0.0, nucleon_dmin
    }};
  else if (species == "Ru")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.04, 0.47, 0., 0., 0.,  0.0, nucleon_dmin
    }};
  else if (species == "Zr")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 4.97, 0.54, 0., 0., 0., 0.0, nucleon_dmin
    }};
  // Read
  else if (species == "Ru_b2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.04, 0.47, 0.18, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_b2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 4.97, 0.54, 0.06, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_b3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.04, 0.47, 0.18, 0.06, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_b3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 4.97, 0.54, 0.06, 0.18, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_new")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.473, 0.16, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_new")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.016, 0.527, 0.0, 0.16, 0.0, 0.0, nucleon_dmin
    }};
    else if (species == "Zr_Wilke")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{    //          R = 4.961 a = 0.544 beta2 = 0.062 beta3 = 0.202
      96, 4.961, 0.544, 0.062, 0.202, 0.0, 0.0, nucleon_dmin
    }};
    else if (species == "Ru_Wilke")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{          //R = 5.065 a = 0.485 beta2 = 0.154 beta3 = 0   
      96, 5.065, 0.485, 0.154, 0.0, 0.0, 0.0, nucleon_dmin
    }};
    else if (species == "Ru_eqA")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{          //R = 5.065 a = 0.485 beta2 = 0.154 beta3 = 0   
      96, 5.09, 0.5, 0.16, 0.0, 0.0, 0.0, nucleon_dmin
    }};
    else if (species == "Zr_eqA")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{    //          R = 4.961 a = 0.544 beta2 = 0.062 beta3 = 0.202
      96, 5.016, 0.5, 0.0, 0.16, 0.0, 0.0, nucleon_dmin
    }};
    else if (species == "Ru_invA")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.53, 0.16, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_invA")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.016, 0.47, 0.0, 0.16, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_round")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.473, 0.0, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_round")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.016, 0.527, 0.0, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_chR")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 4.9, 0.473, 0.16, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_chR")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.2, 0.527, 0.0, 0.16, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Zr_new_base")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.016, 0.527, 0.06, 0.16, 0.0, 0.0, nucleon_dmin
    }};
    else if (species == "Zr_case_6")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.02, 0.52, 0.06, 0.2, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_case_1")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.46, 0.16, 0.0, 0.0, 0.52359877559, nucleon_dmin
    }};
  else if (species == "Ru_case_2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.46, 0.16, 0.0, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_case_3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.46, 0.16, 0.2, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "Ru_case_4")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.46, 0.06, 0.2, 0.0, 0.0, nucleon_dmin
    }};
else if (species == "Ru_case_5")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      96, 5.09, 0.52, 0.06, 0.2, 0.0, 0.0, nucleon_dmin
    }};
else if (species == "Ru_case_test1")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      200, 5.09, 0.46, 0.3, 0.0, 0.0, 0.0, nucleon_dmin
    }};
else if (species == "Ru_case_test2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      200, 5.09, 0.46, 0.3, 0.0, 0.0, 0.52359877559 , nucleon_dmin
    }};
else if (species == "Ru_case_test3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      200, 5.09, 0.46, 0.3, 0.0, 0.0, 1.0471975512, nucleon_dmin
    }};
...
DeformedWoodsSaxonNucleus::DeformedWoodsSaxonNucleus(
    std::size_t A, double R, double a, double beta2, double beta3, double beta4, double gamma, double dmin)
    : MinDistNucleus(A, dmin),
      R_(R),
      a_(a),
      beta2_(beta2),
...
double DeformedWoodsSaxonNucleus::deformed_woods_saxon_dist(
  ...
  auto cos_theta_cu = cos_theta*cos_theta*cos_theta;
  const auto cos_2b = std::cos(2.*phi);
  // spherical harmonics
  using math::double_constants::one_div_root_pi;
  auto Y20 = std::sqrt(5)/4. * one_div_root_pi * (3.*cos_theta_sq - 1.);
  auto Y22 = std::sqrt(15./32.) * one_div_root_pi * (1. - cos_theta_sq) * cos_2b;
  auto Y30 = 1./4. * std::sqrt(7) * one_div_root_pi * (5.*cos_theta_cu - 3.*cos_theta);
  auto Y40 = 3./16. * one_div_root_pi *
             (35.*cos_theta_sq*cos_theta_sq - 30.*cos_theta_sq + 3.);
// cos and sin gamma
    auto cGamma = std::cos(gamma_);
    auto sGamma = std::sin(gamma_);
  // "effective" radius
  auto Reff = R_ * (1. + beta2_*(cGamma*Y20+std::sqrt(2.)*sGamma*Y22) + beta3_*Y30 + beta4_*Y40);
...
void DeformedWoodsSaxonNucleus::sample_nucleons_impl() {
  ...
  // "tilt" angle
  const auto cos_a = random::cos_theta<double>(); //x rot
  const auto sin_a = std::sqrt(1. - cos_a*cos_a);
   THETA = acos(cos_a); //HERE EXTRA DOUBLE
  // "spin" angle
  const auto angle_b = random::phi<double>(); // z rot 1
  const auto angle_c = random::phi<double>(); // z rot 2
  const auto cos_c = std::cos(angle_c);
  const auto sin_c = std::sin(angle_c);
  const auto cos_b = std::cos(angle_b);
  const auto sin_b = std::sin(angle_b); 
  const auto sin_2b = std::sin(2.*angle_b); //new sin stuff
  const auto cos_2b = std::cos(2.*angle_b);
   PHI = angle_b; //HERE EXTRA DOUBLE
  ...
  for (auto&& sample : samples) {
    ...
    do {
      ...
      deformed_woods_saxon_dist(sample.r, sample.cos_theta, sample.phi) //change here also sample???????
    );
  }
  ...
  for (iterator nucleon = begin(); nucleon != end(); ++nucleon, ++sample) {
    ...
    do {
      ...
      //auto phi = random::phi<double>();//remove and use one from above
      // Convert to Cartesian coordinates.
      auto x = r_sin_theta * std::cos(phi);
      auto y = r_sin_theta * std::sin(phi);
      // Rotate.
    ...
    auto x_rot = x*(cos_b*cos_c - cos_a*sin_b*sin_c) + y*(cos_c*sin_b + cos_a*cos_b*sin_c) + z*sin_a*sin_c;
    auto y_rot = -x*(cos_b*sin_c + cos_a*cos_c*sin_b) - y*(sin_b*sin_c - cos_a*cos_b*cos_c) + z*cos_c*sin_a;
    auto z_rot = x*sin_a*sin_b              - y*cos_b*sin_a              + z*cos_a;
