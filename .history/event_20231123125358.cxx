...
void Event::accumulate_TAB(NucleonData& A, NucleonData& B, NucleonCommon& nucleon_common){
  ...
  int ixmin = clip(static_cast<int>((std::max(boundaryA[0], boundaryB[0]) +xymax_)/dxy_), 0, nsteps_-1);
  int ixmax = clip(static_cast<int>((boundaryA[1]+xymax_)/dxy_), 0, nsteps_-1);
  int iymin = clip(static_cast<int>((boundaryA[2]+xymax_)/dxy_), 0, nsteps_-1);
  int iymax = clip(static_cast<int>((boundaryA[3]+xymax_)/dxy_), 0, nsteps_-1);
